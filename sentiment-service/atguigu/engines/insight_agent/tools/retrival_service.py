...
from atguigu.engines.contract.evidence import EvidenceRecord, EvidenceDocument, RetrievalMeta
from atguigu.engines.insight_agent.tools.db.repository import DatabaseSearchRepository
from atguigu.engines.insight_agent.tools.search_results import SearchHit, SearchResult
from atguigu.engines.insight_agent.tools.vector.repository import VectorSearchRepository
...
class RetrivalService:
    ...
    def __init__(self):
        ...
        self.vector_repository = VectorSearchRepository() if get_settings().INSIGHT_VECTOR_ENABLED else None
    ...
    async def _retrival_vector_evidence(self, query: str) -> list[EvidenceRecord]:
        ...
        # 2. (开启新线程)检索
        search_hits: list[SearchHit] = await asyncio.to_thread(self.vector_repository.vector_call, query, 50,
                                                               build_filter_expr())
...
async def main_test():
    ...
    results = await  retrival_service.retrival_evidence(query="高考难不难")
