...
from atguigu.engines.contract.evidence import EvidenceDocument, Engagement
from atguigu.engines.insight_agent.tools.db_connection import DataBaseConnectionManager, connection_manager
from atguigu.engines.insight_agent.tools.search_results import SearchResult
from atguigu.engines.insight_agent.tools.sql import db_sql_statement
...
class DatabaseSearchRepository:
    ...
    def _mapping_to_document(self, row: dict[str, Any]) -> EvidenceDocument:
        return EvidenceDocument(
            ...
            engagement={
                field.name: row[f'eng_{field.name}'] for field in fields(Engagement)  # type: ignore
            },
...
async def main():
    ...
    limit = 10
    try:
        result: SearchResult = await repo.retrival_db(search_term=term, limit=limit)
